...
import Page1ByYear from '@/components/CompAdmin/results/Page1ByYear';
...
const PageResults = () => {
  ...
  return (<TabContext value={value}>
      ...
      <TabList 
      >
        <Tab icon={<School />} onClick={() => setValue("1")} value={"1"} label="By Year" />
        <Tab icon={<IconBook />} onClick={() => setValue("2")} value={"2"} label="By Specialty" />
        <Tab icon={<IconBrandEmber />} onClick={() => setValue("3")} value={"3"} label="By Course" />
        <Tab icon={<IconBrandEmber />} onClick={() => setValue("4")} value={"4"} label="Results" />
        <Tab icon={<IconBrandEmber />} onClick={() => setValue("5")} value={"5"} label="Generate" />
      </TabList>

      
      <TabPanel sx={{ m: 0, p: 0}} value="1">
        <PageContainer title="Dashboard" description="this is Year"><Page1ByYear setSelectedNumber={setValue} /></PageContainer>
      </TabPanel>
